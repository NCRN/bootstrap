...
window.onload = function () { // wait for load in a dumb way because B-0
  ...
  function generateCSS() {
    ...
    var bsLessSource    = generateLESS('bootstrap-nps.less', lessFileIncludes, vars)
    var themeLessSource = generateLESS('theme.less',     lessFileIncludes, vars)

    try {
      compileLESS(bsLessSource, 'nps-bootstrap', result)
      compileLESS(themeLessSource, 'nps-bootstrap-theme', result)
    } catch (err) {
      return showError('<strong>Ruh roh!</strong> Could not parse less files.', err)
    }
