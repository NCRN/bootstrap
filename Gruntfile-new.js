...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    csslint: {
      ...
      src: [
        'dist/css/nps-bootstrap.css',
        'dist/css/nps-bootstrap-theme.css',
    ...
    uglify: {
      bootstrap: {
        ...
        dest: 'dist/js/nps-<%= pkg.name %>.min.js'
      },
      customize: {
        options: {
          banner: '/*!\n' +
    ...
    less: {
      compileCore: {
        options: {
          ...
          sourceMapFilename: 'dist/css/nps-<%= pkg.name %>.css.map'
        },
        files: {
          'dist/css/nps-<%= pkg.name %>.css': 'less/bootstrap.less'
        }
      },
      compileTheme: {
        options: {
          ...
          sourceMapFilename: 'dist/css/nps-<%= pkg.name %>-theme.css.map'
        },
        files: {
          'dist/css/nps-<%= pkg.name %>-theme.css': 'less/theme.less'
        }
      },
      minify: {
        options: {
        ...
        files: {
          'dist/css/nps-<%= pkg.name %>.min.css': 'dist/css/nps-<%= pkg.name %>.css',
          'dist/css/nps-<%= pkg.name %>-theme.min.css': 'dist/css/nps-<%= pkg.name %>-theme.css'
        }
      }
    },
    ...
    usebanner: {
      dist: {
        ...
        files: {
          src: [
            'dist/css/nps-<%= pkg.name %>.css',
            'dist/css/nps-<%= pkg.name %>.min.css',
            'dist/css/nps-<%= pkg.name %>-theme.css',
            'dist/css/nps-<%= pkg.name %>-theme.min.css',
          ]
        }
      }
    },
    ...
    csscomb: {
      sort: {
        ...
        files: {
          'dist/css/nps-<%= pkg.name %>.css': ['dist/css/nps-<%= pkg.name %>.css'],
          'dist/css/nps-<%= pkg.name %>-theme.css': ['dist/css/nps-<%= pkg.name %>-theme.css']
        }
      }
    },
    ...
    jade: {
      compile: {
        options: {
          pretty: true,
          data: function () {
            var filePath = path.join(__dirname, 'less/variables-nps.less');
